...
export const ThemeMenu = () => {
    ...
    return (
        <div>
            <button ref={menu_toggle_ref} className="dropdown__toggle r-btn" onClick={() => setActiveMenu()}>
                <BiPalette size={35} />
            </button>
                            ...
                            mode_settings.map((item, index) => (
                                <li key={index} onClick={() => setMode(item)}>
                                    <div className={`mode-list__color ${item.background}`}>
                                        {item.id === currMode ? <GiCheckMark color='green' style={{ padding: 2 }} /> : null}
                                    </div>
                                    <span>{item.name}</span>
                                </li>
                            ))
                    ...
                    <ul className="mode-list">
                        {
                            color_settings.map((item, index) => (
                                <li key={index} onClick={() => setColor(item)}>
                                    <div className={`mode-list__color ${item.background}`}>
                                        {item.id === currColor ? <GiCheckMark color='black' style={{ padding: 2 }} /> : null}
                                    </div>
                                    <span>{item.name}</span>
                                </li>
                            ))
